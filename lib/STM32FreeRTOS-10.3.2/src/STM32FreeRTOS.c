...
{
  (void)pxTask;
  // BUG FIX (diagnostic) : les deux taches sont maintenant nommees a la
  // creation ("myTask" / "lvglTask", voir lvglDrivers.cpp). On garde le
  // signal habituel "3 clignotements" = depassement de pile pour myTask
  // (rien ne change si c'est elle), mais on ajoute un signal DIFFERENT et
  // facilement reconnaissable (6 clignotements) si c'est lvglTask, pour
  // savoir laquelle des deux deborde sans avoir besoin d'un debogueur.
  if (pcTaskName != 0 && strcmp(pcTaskName, "lvglTask") == 0)
    errorBlink(6);
  else
    errorBlink(3);
}
